...
if [ ! -d "assets/js" ] || [ ! -d "assets/css" ] || \
   [ -z "$(find assets/js -maxdepth 1 -name '*.min.js' -print -quit)" ] || \
   [ -z "$(find assets/css -maxdepth 1 -name '*.min.css' -print -quit)" ]; then
  echo "Asset build failed; expected minified JS/CSS outputs under assets/." >&2
  exit 1
fi
